...
const StarRating = () => {
  ...
  useEffect(() => {
    setRating((Math.random() * 2 + 3).toFixed(1)); // 3.0-5.0 rating
  }, []);
...
const ProductCard = ({ product, view }) => {
  const { id, image, name, price, discount, orders, description } = product;
  
  return view === "grid" ? (
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md overflow-hidden border border-gray-100 p-4 flex flex-col transition-all duration-200 h-full">
    ...
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md overflow-hidden border border-gray-100 py-4 px-6 flex flex-col sm:flex-row items-start gap-6 transition-all duration-200">
      <div className="w-full sm:w-40 h-40 flex-shrink-0 bg-gray-50 rounded-lg flex items-center justify-center">
        <img src={image} alt={name} className="max-w-full max-h-full object-contain p-2" loading="lazy" />
      </div>
