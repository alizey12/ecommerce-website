...
const ProductsListing = ({ products = [], }) => {
  ...
  const [sortedProducts, setSortedProducts] = useState([...products]);  const [currentPage, setCurrentPage] = useState(1);
      ...
      <main className="w-full">
        {view === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:container md:mx-auto sm:gap-6 w-full">
            {sortedProducts.map(({ id, image, name, price, discount, orders }) => (
              <div 
                key={id} 
