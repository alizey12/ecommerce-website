...
const ProductsListing = ({ 
  ...
  const StarRating = ({ rating }) => (
    <div className="flex items-center gap-1">
      {[...Array(5)].map((_, index) => (
        <FaStar key={index} className={`text-orange-500 ${index < rating ? "filled" : ""}`} />
      ))}
      ...
      <main className="w-full">
        {view === "grid" ? (
          ...
          <div className="flex flex-col gap-4 w-full">
            {currentProducts.map(({ id, image, name, price, discount, orders, description, rating }) => (
              <div 
                key={id} 
              ...
              >
                ...
                <div className="flex flex-col justify-between flex-grow w-full">
                  ...
                  <div className="mt-2">
        <StarRating rating={rating} /> {/* Use the rating from the product */}
      </div>
