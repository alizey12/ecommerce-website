    ...
    return (
      <div className="flex items-center gap-1">
        {[...Array(5)].map((_, index) => (
          <FaStar 
            key={index} 
            className={`text-orange-500 ${index < validRating ? "filled" : ""}`} 
          />
        ))}
        <span className="text-orange-500 text-sm ml-1">{validRating.toFixed(1)}</span>
      </div>
    );
  };
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
              ...
              >
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {discount && (
                      <span className="text-gray-500 line-through text-sm">${discount}</span>
                    )}
                  </div>
                  <div className="mt-2">
            ...
            {currentProducts.map(({ id, image, name, price, discount, orders, description }) => (
              ...
              >
                ...
                <div className="flex flex-col justify-between flex-grow w-full">
                  ...
                  <div className="mt-2">
                    <StarRating rating={manualRating} />
                  </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center mt-8 gap-1 sm:gap-2">
          <button onClick={prevPage} disabled={currentPage === 1} className="p-2 rounded-md text-gray-700 hover:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed">
            <FaChevronLeft />
          </button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
            <button key={number} onClick={() => paginate(number)} className={`w-8 h-8 rounded-md transition-colors ${currentPage === number ? "bg-blue-600 text-white" : "text-gray-700 hover:bg-gray-100"}`}>
              {number}
            </button>
          ))}
          <button onClick={nextPage} disabled={currentPage === totalPages} className="p-2 rounded-md text-gray-700 hover:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed">
            <FaChevronRight />
          </button>
        </div>
      )}
