...
const ProductsListing = ({ 
  ...
  manualRating = 4 // Default to 4 stars if not provided
}) => {
  ...
  const StarRating = ({ rating }) => {
    // Use a default rating of 4 if the provided rating is undefined or null
    const finalRating = rating != null ? rating : 4;
    ...
    return (
      <div className="flex items-center gap-1">
        {Array.from({ length: 5 }).map((_, index) => (
          <FaStar
            key={index}
            className={`text-orange-500 ${index < finalRating ? "filled" : "text-gray-300"}`}
          />
        ))}
        <span className="text-orange-500 text-sm ml-1">{finalRating.toFixed(1)}</span>
      </div>
    );
  };
