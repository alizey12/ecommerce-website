...
export default function HeroDeals() {
 ...
 <div className="bg-white shadow-md mb-5 w-full h-full">
  <div className="grid grid-cols-4 "> {/* No gap between columns */}
    {/* Left Banner */}
    <div className="relative w-64 h-64 col-span-1">
      {/* Background Image */}
      <img
      ...
      {/* Overlay Content */}
      <div className="absolute top-0 left-0 w-full h-full p-5 flex flex-col justify-start">
        <h2 className="font-bold text-2xl pl-2 pr-12 text-black">Home and Outdoor</h2>
    ...
    {/* Items Grid */}
    <div className="col-span-3 grid grid-cols-4 gap-0"> {/* No gap between items */}
      {items.map((item) => (
        <div
          key={item.id}
          className="border flex flex-col justify-between h-32 w-full items-center text-center"
        >
 ...
 <div className="w-full md:w-3/4 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4">  <div className="grid grid-cols-4 "> {/* No gap between columns */}
    {/* Left Banner */}
    <div className="relative w-64 h-64 col-span-1">
      {/* Background Image */}
      <img
        ...
        {/* Add `block` to remove inline gap */}
      

      {/* Overlay Content */}
      <div className="absolute top-0 left-0 w-full h-full p-5 flex flex-col justify-start bg-opacity-50 rounded-lg">
        <h2 className="font-bold text-2xl pl-2 pr-12 text-black">Home and Outdoor</h2>
        <button className="mt-4 bg-white text-gray-900 font-medium w-32 px-2 py-2 rounded-md shadow">
          Source now
        </button>
    ...
    {/* Items Grid */}
    <div className="grid flex-1 grid-cols-4 col-span-3 gap-0"> {/* No gap between items */}
      {electronicsItems.map((item) => (
        <div
          key={item.id}
          className="border flex flex-col p-4 justify-between w-full min-w-[100px] h-32 items-center text-center shadow-md"
        >
          <div>
            <p className="font-medium text-sm m-0">{item.name}</p> {/* Reset margin */}
            <p className="text-gray-500 text-sm m-0">From {item.price}</p> {/* Reset margin */}
          </div>
          <img
            src={item.image}
            alt={item.name}
            className="h-16 object-contain block"  />{/* Add `block` to remove inline gap */}
